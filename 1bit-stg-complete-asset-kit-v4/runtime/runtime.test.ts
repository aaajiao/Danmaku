import {
  ...
  canonicalizeRoomId,
  CrossRunArchive,
  CrossRunRestoreMachine,
  DirectionalOverrideMachine,
  EventId,
  EventTrace,
  EvidenceLedger,
  FeedbackRouter,
  FlowerIntensityResolver,
  GazeMachine,
  GrazeAwardRegistry,
  PlayerDamageMachine,
  ProjectileLifecycle,
  REFERENCE_FEEDBACK_BINDINGS,
  RoomTransitionMachine,
  SnapshotMachine,
  SnapshotRecord,
  WeatherMachine,
} from "./index.js";
...
function throws(run: () => void, pattern: RegExp, message: string): void {
  let error: unknown;
  try {
    run();
  } catch (caught) {
...
function byId(trace: EventTrace, id: EventId) {
  return trace.canonicalEvents().filter((event) => event.id === id);
}

function requirePreset(id: string): AccessibilityProfile {
  const preset = ACCESSIBILITY_PRESETS[id];
  if (!preset) throw new Error(`missing accessibility preset: ${id}`);
  return preset;
}

function testPlayerDamageAuthorityAndLeases(): void {
  const trace = new EventTrace();
  const player = new PlayerDamageMachine(trace, {
    maxHealth: 2,
    initialLives: 2,
...
function testFatalRunEndHasNoRespawnRace(): void {
  const trace = new EventTrace();
  const player = new PlayerDamageMachine(trace, {maxHealth: 1, initialLives: 1});
...
function testEntityOwnedProjectileAndGrazeOnce(): void {
  const trace = new EventTrace();
  const projectile = new ProjectileLifecycle(trace, "bullet-7", {armDelayMs: 90, residueMs: 220});
  const evidence = new EvidenceLedger(trace);
  const graze = new GrazeAwardRegistry(trace, evidence);
...
function testGazeSustainedClampAndDelayedRelease(): void {
  const trace = new EventTrace();
  const gaze = new GazeMachine(trace, {acquireMs: 500, releaseDelayMs: 300});
...
function testFlowerPriority(): void {
  const trace = new EventTrace();
  const flower = new FlowerIntensityResolver(trace);
...
function testDirectionalOverrideIsLocalAndMaterial(): void {
  const trace = new EventTrace();
  const evidence = new EvidenceLedger(trace, 8);
  const override = new DirectionalOverrideMachine(trace, evidence, {
    chargeMs: 100,
    activeMs: 200,
    sedimentMs: 50,
    cooldownMs: 100,
...
function testBossPhaseKinds(): void {
  const trace = new EventTrace();
  const boss = new BossPhaseMachine(trace, "absolute-reader", [
...
function testWeatherLargeDelta(): void {
  const trace = new EventTrace();
  const weather = new WeatherMachine(trace);
  ...
  deepEqual(
    ["weather.active.begin", "weather.aftermath.begin", "weather.cooldown.begin", "weather.complete"].map((id) =>
      trace.canonicalEvents().find((event) => event.id === id)?.simulationTimeMs,
    ),
    [150, 250, 325, 350],
    "weather boundaries retain exact simulation times",
  );
}
...
function testSnapshotAndCrossRunAreSeparate(): void {
  const trace = new EventTrace();
  const snapshot = new SnapshotMachine(trace);
  ...
  const archive = new CrossRunArchive(trace);
  ...
  deepEqual(
    byId(trace, "snapshot.complete").map((event) => event.simulationTimeMs),
    [1630],
    "snapshot completion timing is authoritative",
  );

  const restore = new CrossRunRestoreMachine(trace, "run-0032");
  ...
  deepEqual(
    ["overrideScar.rehydrate", "deathTrace.rehydrate", "burnIn.rehydrate", "ghost.residue.write"].map((id) =>
      byId(trace, id as EventId)[0]?.payload.recordType),
    ["overrideScar", "deathTrace", "burnIn", "ghostResidue"],
    "four material memory types remain semantically separate",
  );
  equal(byId(trace, "ghost.replay.begin")[0]?.payload.routeDurationMs, 960, "ghost begins with actual route duration");
}

function testRoomTransitionAtomicSwap(): void {
  const trace = new EventTrace();
  equal(canonicalizeRoomId("INFO_OVERFLOW"), "INFORMATION", "legacy room id is read as a migration alias");
  const rooms = new RoomTransitionMachine(trace, "INFORMATION");
...
function runReferenceScenario(profile: AccessibilityProfile): {trace: string; cueIds: readonly string[]} {
  const trace = new EventTrace();
  const gaze = new GazeMachine(trace, {acquireMs: 100, releaseDelayMs: 100});
  gaze.observe({skyEyeVisible: true, pitchDegrees: 60, alignment: 1}, 0);
  gaze.advance(100);

  const projectile = new ProjectileLifecycle(trace, "scenario-bullet", {armDelayMs: 100, residueMs: 100});
  const evidence = new EvidenceLedger(trace, 8);
  const graze = new GrazeAwardRegistry(trace, evidence);
  ...
  const room = new RoomTransitionMachine(trace, "POLARIZED");
  room.request("FORCED_ALIGNMENT", 1500);
  room.advance(2500);

  const restore = new CrossRunRestoreMachine(trace, "scenario-next-run");
  restore.begin(SAMPLE_RECORD, 2600);
  restore.advance(5000);

  const router = new FeedbackRouter(REFERENCE_FEEDBACK_BINDINGS);
...
function testAccessibilityGameplayTraceParity(): void {
...
function testFeedbackIsReadOnlyAndAxesAreIndependent(): void {
  const trace = new EventTrace();
  const event = trace.emit("room.transition.begin", 0, {fromRoom: "INFORMATION", toRoom: "FORCED_ALIGNMENT"});
  const router = new FeedbackRouter(REFERENCE_FEEDBACK_BINDINGS);
  const monoOnly: AccessibilityProfile = {
    ...requirePreset("full"),
    binaural: "mono",
  };
