...
当前运行时按语义选择五姿态：phase 开场短暂 `prepare`；子弹真正进入 BulletSystem 后进入 `cast`，随后短暂 `expand`；当前 phase 的剩余血量或时间进入最后八分之一时使用 `close`；其余为 `idle`。受击仍是独立 hit flash，不替代姿态。所有选择只读固定 tick 的模拟事实，绝不通过动画回调发射子弹。
...
专用 portrait atlas 不单独建立：运行时按十位角色各自的固定 crop/anchor，直接从
实际 v4 actor atlas 的主角 neutral 或 Boss cast 帧裁取脸、心核与手部。人物身份因此
统一且没有占位头像；第三方角色继续使用 pack portrait 或确定性 fallback。只有未来
独立绘制的 UI 母版能明显改善读脸时才拆分，且不能复制一套新的角色身份。
...
| 命中材质 | 16 类敌人和 5 位 Boss 全部显式绑定 `surface/skeleton/mycelium/heart`；四套 8 帧反馈同时有 procedural floor 与 v4 native strip；致死命中让位死亡 FX，激光只限频视觉层而不降伤害 | 浏览器逐材质确认膜纹、骨节、菌丝回缩和心核压缩在实战尺寸仍能区分 |
| 对话头像 | 主角与五位 Boss 已按逐人固定 anchor 从同一 Ghost actor 图集取近景；第三方 pack speaker 仍走其 portrait/fallback | 浏览器检查脸、心核和姓名板在四种 shader 上均可读 |
...
| 原创 v4 包 | 默认包的 72 个 bullet 名、34 个 effect/player FX（含四套材质反馈、五组专属 option 与五组专属 Bomb）、11 个 laser、13 个 missile、10 个 pickup、五档心翼与 HUD 均为项目自有像素；敌弹已有实体核/骨白 keyline，玩家弹为身份色 keyline；分类图集使用确定性 shelf packing | 浏览器逐关检查强弹层级、主角状态身份和 Normal/Lunatic 密集弹幕负空间 |
