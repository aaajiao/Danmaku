...
const DEFERRED_FIRST_CONTINUATION_NEXT_READ_INSTALL = Symbol(
  "deferred-first-continuation-next-read-install",
);
...
const ACTIVE_DORMANT_SUCCESSOR_TRANSFER_BY_RUN_STATE = new WeakMap<
  ...
  PreparedCanonicalRunFirstContinuationDormantSuccessorTransfer
>();
const RUN_STATE_BY_FIRST_CONTINUATION_SUCCESSOR_OWNER = new WeakMap<
  object,
  CanonicalRunCombatState
>();
...
export interface CanonicalRunOccurrenceMaterialCarryoverSnapshot {
...
export interface PreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer {
  readonly [preparedSuccessorMaterialTransferBrand]:
    "PreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer";
}

export interface PreparedCanonicalRunFirstContinuationSuccessorMaterialTransferView {
  readonly authority:
  ...
  readonly materialCount: number;
  readonly poolUsage: ProjectilePoolUsage;
  readonly predecessorMaterialLease: "retire-on-commit";
  readonly gameplayAuthority: "released";
  readonly roomCompletion: "withheld";
  readonly roomHandoff: "withheld";
  readonly nextOccurrenceAdmission:
...
export interface CanonicalRunFirstContinuationSuccessorMaterialHoldResult {
  readonly runCombat: CanonicalRunCombatStateSnapshot;
...
export interface CanonicalRunFirstContinuationNextOccurrenceDormantOwnerSnapshot {
  ...
  readonly projectilePoolAudit: readonly ProjectilePoolAuditRecord[];
...
export interface CanonicalRunFirstContinuationNextOccurrenceMaterialHoldResult {
  readonly runCombat: CanonicalRunCombatStateSnapshot;
  readonly material: CanonicalRunFirstContinuationNextOccurrenceMaterialCarryoverSnapshot;
  readonly flushedEvents: readonly CanonicalGameplayEvent[];
}
...
interface RunOccurrenceMaterialCarryoverRecord {
  readonly runState: CanonicalRunCombatState;
  readonly eventBus: CanonicalEventBus;
  readonly predecessor: CanonicalRoomThresholdMaterialCarryover;
  readonly predecessorOwner: object;
  readonly kernel: CanonicalCombatKernel;
  readonly sourcePatternId: string;
  readonly sourceOccurrenceId: string;
  readonly transferredAtTick120: number;
...
interface PreparedSuccessorMaterialTransferRecord {
  readonly runState: CanonicalRunCombatState;
  readonly eventBus: CanonicalEventBus;
  readonly predecessor: CanonicalRoomThresholdMaterialCarryover;
  readonly predecessorOwner: object;
  ...
  readonly view: PreparedCanonicalRunFirstContinuationSuccessorMaterialTransferView;
  status: "prepared" | "committed" | "cancelled" | "failed";
}
...
>();
const RUN_OCCURRENCE_MATERIAL_CARRYOVERS = new WeakMap<
  CanonicalRunOccurrenceMaterialCarryover,
  RunOccurrenceMaterialCarryoverRecord
>();
const PREPARED_SUCCESSOR_MATERIAL_TRANSFERS = new WeakMap<
  PreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer,
  PreparedSuccessorMaterialTransferRecord
>();
...
export class CanonicalRunCombatState {
  ...
  flushTick(tick120Value: number): readonly CanonicalGameplayEvent[] {
    ...
    if (binding?.phase === "transition") {
    ...
    } else if (binding?.phase === "successor-material") {
      const room = RoomTransitionAuthority.prototype.snapshot.call(binding.roomTransition);
      const predecessor = binding.carryover?.snapshot() ?? null;
      const materialOwner = binding.successorMaterial;
      const material = materialOwner?.snapshot() ?? null;
      const combat = binding.successorFinalCombat;
      if (
        binding.successorOwner === null
        || binding.successorPlan === null
        ...
        const error = new Error(
          "transferred successor material lost its collisionless material/room synchronization",
        );
        internals.fault = error;
        throw error;
      }
    } else if (binding?.phase === "successor-next-pre-read") {
      const room = RoomTransitionAuthority.prototype.snapshot.call(binding.roomTransition);
      const predecessor = binding.carryover?.snapshot() ?? null;
      const materialOwner = binding.successorMaterial;
      const material = materialOwner?.snapshot() ?? null;
      ...
      if (
        ...
        || material === null
        || material.tick120 !== tick120
        || material.sourcePatternId !== sourceCombat.patternId
        || material.sourceOccurrenceId !== sourceCombat.occurrenceId
        || material.poolUsage.liveColliders !== 0
        || material.projectiles.some((projectile) =>
          projectile.state !== "residue" || projectile.collisionEnabled)
        ...
        || sourceCombat.projectiles.some((projectile) =>
          projectile.state !== "residue" || projectile.collisionEnabled)
        ...
        || DORMANT_SUCCESSOR_POOL_CLASS_ORDER.some((poolClass) =>
          material.poolUsage.allocatedSlots[poolClass]
            !== evaluation.carryover.allocatedSlots[poolClass])
        || player.tick120 !== tick120
        || player.state !== "alive"
        ...
        const error = new Error(
          "next occurrence pre-READ material, room, and Run tick lost sealed synchronization",
        );
        internals.fault = error;
        throw error;
      }
      ...
      const releaseRequested = binding.phase === "successor-next-release-requested";
      const room = RoomTransitionAuthority.prototype.snapshot.call(binding.roomTransition);
      const predecessor = binding.carryover?.snapshot() ?? null;
      const materialOwner = binding.successorMaterial;
      const material = materialOwner?.snapshot() ?? null;
      ...
      const plan = binding.nextSuccessorPlan;
      const evaluation = binding.nextSuccessorAdmission;
      const combat = binding.nextSuccessorKernel?.snapshot() ?? null;
      const player = internals.player.snapshot();
      const override = internals.override.snapshot();
      if (
        binding.nextSuccessorOwner === null
        ...
        || materialOwner === null
        || binding.predecessorMaterialLeaseRetiredAtTick120 === null
        || internals.activeOccurrenceId !== plan.occurrence.occurrenceId
        || (releaseRequested
          ? internals.pendingReleaseOccurrenceId !== plan.occurrence.occurrenceId
            || combat?.patternComplete !== true
            || combat.digitalBodiesDrained !== true
            || combat.poolUsage.liveColliders !== 0
            || combat.projectiles.some((projectile) =>
        ...
        const error = new Error(
          "next occurrence READ/release combat, retained material, room, and Run tick lost sealed synchronization",
        );
        internals.fault = error;
        throw error;
      }
    ...
    } else if (binding?.phase === "successor-next-material") {
      const room = RoomTransitionAuthority.prototype.snapshot.call(binding.roomTransition);
      const transitionMaterial = binding.carryover?.snapshot() ?? null;
      const sourceMaterialOwner = binding.successorMaterial;
      const sourceMaterialRecord = sourceMaterialOwner === null
        ? undefined
        : RUN_OCCURRENCE_MATERIAL_CARRYOVERS.get(sourceMaterialOwner);
      const sourceCombat = binding.successorKernel === null
        ? null
        : CanonicalCombatKernel.prototype.snapshot.call(binding.successorKernel);
      const materialOwner = binding.nextSuccessorMaterial;
      const materialRecord = materialOwner === null
        ? undefined
        : NEXT_OCCURRENCE_MATERIAL_CARRYOVERS.get(materialOwner);
      const combat = binding.nextSuccessorKernel === null
        ? null
        : CanonicalCombatKernel.prototype.snapshot.call(binding.nextSuccessorKernel);
      const plan = binding.nextSuccessorPlan;
      const evaluation = binding.nextSuccessorAdmission;
      const player = internals.player.snapshot();
      const override = internals.override.snapshot();
      if (
        ...
        || binding.nextSuccessorFinalCombat === null
        || materialOwner === null
        || materialRecord?.runState !== this
        || materialRecord.eventBus !== internals.bus
        || materialRecord.predecessor !== sourceMaterialOwner
        || materialRecord.predecessorRecord !== sourceMaterialRecord
        || materialRecord.predecessorOwner !== binding.nextSuccessorOwner
        || materialRecord.transitionPredecessor !== binding.carryover
        || materialRecord.kernel !== binding.nextSuccessorKernel
        || materialRecord.sourcePatternId !== plan.occurrence.patternId
        || materialRecord.sourceOccurrenceId !== plan.occurrence.occurrenceId
        || binding.successorMaterialCapacityLeaseRetiredAtTick120
          !== materialRecord.transferredAtTick120
        || binding.successorOwner === null
        || binding.successorPlan === null
        || binding.successorReservation === null
        || binding.successorKernel === null
        || binding.successorFinalCombat === null
        || sourceMaterialOwner === null
        || sourceMaterialRecord?.runState !== this
        || sourceMaterialRecord.eventBus !== internals.bus
        || sourceMaterialRecord.kernel !== binding.successorKernel
        || sourceMaterialRecord.predecessor !== binding.carryover
        || binding.predecessorMaterialLeaseRetiredAtTick120 === null
        || internals.activeOccurrenceId !== null
        || internals.pendingReleaseOccurrenceId !== null
        || binding.materialRoomEventCount === null
        ...
        || sourceCombat.projectiles.length !== 0
        || sourceCombat.poolUsage.liveColliders !== 0
        || binding.successorFinalCombat.tick120 !== tick120
        || combat === null
        || combat.tick120 !== tick120
        || combat.patternId !== plan.occurrence.patternId
        || combat.occurrenceId !== plan.occurrence.occurrenceId
        ...
        || combat.projectiles.some((projectile) =>
          projectile.state !== "residue" || projectile.collisionEnabled)
        || binding.nextSuccessorFinalCombat.tick120 !== tick120
        || !nextOccurrencePoolUsagesFitAdmission(
          sourceCombat.poolUsage,
          combat.poolUsage,
          evaluation,
        )
        || combat.playerPosition.x !== internals.currentPlayerPosition.x
        ...
        const error = new Error(
          "next occurrence transferred material lost its four-lineage material/room synchronization",
        );
        internals.fault = error;
        throw error;
      }
...
export class CanonicalCombatKernel {
  ...
  private deferredFirstContinuationNextReadInstallPending = false;
  ...
  constructor(
    ...
    const deferredFirstContinuationNextReadInstall =
      installMode === DEFERRED_FIRST_CONTINUATION_NEXT_READ_INSTALL;
    const deferredSharedInstall = deferredRoomThresholdInstall
      || deferredFirstContinuationReadInstall
      || deferredFirstContinuationNextReadInstall;
    if (installMode !== undefined && !deferredSharedInstall) {
      throw new Error("canonical combat received an unknown internal install mode");
    }
    if (
      ((deferredFirstContinuationReadInstall || deferredFirstContinuationNextReadInstall)
  ...
  [DEFERRED_FIRST_CONTINUATION_NEXT_READ_INSTALL](
    runState: CanonicalRunCombatState,
    nextOwner: CanonicalRunFirstContinuationNextOccurrenceDormantOwner,
    plan: CanonicalRunFirstContinuationNextOccurrencePlanPayload,
    ...
    const material = materialOwner?.snapshot() ?? null;
    const predecessor = binding?.carryover?.snapshot() ?? null;
    const sourceCombat = binding?.successorFinalCombat ?? null;
    const room = binding === undefined
      ? null
      : RoomTransitionAuthority.prototype.snapshot.call(binding.roomTransition);
    const player = internals.player.snapshot();
    ...
    if (
      ...
      || binding.nextSuccessorPlan !== plan
      || binding.nextSuccessorAdmission !== evaluation
      || binding.nextSuccessorKernel !== null
      || binding.nextSuccessorFinalCombat !== null
      || materialOwner === null
      || materialRecord?.runState !== runState
      || binding.successorKernel !== materialRecord.kernel
      || binding.successorFinalCombat === null
      || binding.successorPlan === null
      || binding.successorReservation === null
      || binding.carryover !== materialRecord.predecessor
      || this.pattern.id !== plan.occurrence.patternId
      || this.occurrenceId !== plan.occurrence.occurrenceId
      || this.options.seed !== plan.occurrence.resolvedSeed.value
      || this.options.startTick120 !== internals.currentTick120
      ...
      || !evaluation.admissible
      || evaluation.evaluatedAtTick120 !== plan.plannedAtTick120
      || evaluation.targetRoom !== plan.targetRoom
      || evaluation.occurrence.occurrenceId !== this.occurrenceId
      || evaluation.occurrence.patternId !== this.pattern.id
      || evaluation.occurrence.difficulty !== plan.occurrence.difficulty
      ...
      throw new Error(
        "deferred next-occurrence READ install lost its exact plan and retained material lease",
      );
    }
    this.focused = internals.focused;
    internals.claimedOccurrenceIds.add(this.occurrenceId);
    ...
    binding.phase = "successor-next-read";
    this.deferredFirstContinuationNextReadInstallPending = false;
  }
  ...
  [SEALED_FIRST_CONTINUATION_NEXT_READ_ADVANCE](
    nextOwner: CanonicalRunFirstContinuationNextOccurrenceDormantOwner,
    input: CanonicalCombatStepInput,
  ): CanonicalCombatSnapshot {
    const binding = EXT013_ROOM_THRESHOLD_RUN_BINDINGS.get(this.runState);
    ...
    if (
      ...
      || this.occurrenceId !== plan.occurrence.occurrenceId
      || this.options.seed !== plan.occurrence.resolvedSeed.value
      || this.options.startTick120 !== successorBoundaryTicks(plan).readStartTick120
      || this.options.roomId !== plan.targetRoom
      || this.options.difficulty !== plan.occurrence.difficulty
    ) {
      throw new Error(
        "sealed next-occurrence READ advance requires its exact active owner and admission",
      );
    }
    return this.advanceTickWithLock(input);
  }
  ...
  [SEALED_FIRST_CONTINUATION_NEXT_MATERIAL_ADVANCE](
    ...
    const plan = binding?.nextSuccessorPlan ?? null;
    const evaluation = binding?.nextSuccessorAdmission ?? null;
    const transferredMaterialRecord = NEXT_OCCURRENCE_MATERIAL_CARRYOVERS.get(
      materialOwner as CanonicalRunFirstContinuationNextOccurrenceMaterialCarryover,
    );
    const occurrenceOwnerLease = binding?.phase === "successor-next-tail"
      && binding.nextSuccessorOwner === materialOwner
      && binding.nextSuccessorMaterial === null
      && binding.successorMaterialCapacityLeaseRetiredAtTick120 === null;
    const transferredMaterialLease = binding?.phase === "successor-next-material"
      && binding.nextSuccessorMaterial === materialOwner
      && transferredMaterialRecord?.runState === this.runState
      && transferredMaterialRecord.kernel === this
      && transferredMaterialRecord.predecessorOwner === binding.nextSuccessorOwner
      && binding.successorMaterialCapacityLeaseRetiredAtTick120
        === transferredMaterialRecord.transferredAtTick120;
    ...
    if (
      ...
      || this.deferredFirstContinuationNextReadInstallPending
      || binding === undefined
      || (!occurrenceOwnerLease && !transferredMaterialLease)
      || binding.nextSuccessorKernel !== this
      || binding.nextSuccessorFinalCombat === null
      || plan === null
      || evaluation === null
  ...
  [SEALED_FIRST_CONTINUATION_MATERIAL_ADVANCE](
    ...
    const before = CanonicalCombatKernel.prototype.snapshot.call(this);
    const occurrenceOwnerLease =
      (binding?.phase === "successor-tail" || binding?.phase === "successor-complete")
      && binding.successorOwner === materialOwner
      && binding.successorMaterial === null;
    const transferredMaterialRecord = RUN_OCCURRENCE_MATERIAL_CARRYOVERS.get(
      materialOwner as CanonicalRunOccurrenceMaterialCarryover,
    );
    const transferredMaterialLease = (
      ...
      && transferredMaterialRecord?.runState === this.runState
      && transferredMaterialRecord.kernel === this;
    const nextMaterialOwner = binding?.nextSuccessorMaterial ?? null;
    const nextMaterialRecord = nextMaterialOwner === null
      ? undefined
      : NEXT_OCCURRENCE_MATERIAL_CARRYOVERS.get(nextMaterialOwner);
    const retiredPredecessorMaterialLease = binding?.phase === "successor-next-material"
      && binding.successorMaterial === materialOwner
      && nextMaterialRecord?.runState === this.runState
      && nextMaterialRecord.predecessor === materialOwner
      && nextMaterialRecord.predecessorRecord === transferredMaterialRecord
      && nextMaterialRecord.transitionPredecessor === binding.carryover
      && nextMaterialRecord.kernel === binding.nextSuccessorKernel
      && binding.successorMaterialCapacityLeaseRetiredAtTick120
        === nextMaterialRecord.transferredAtTick120;
    if (
      this.advanceLocked
      || !this.sharedRunState
      || !this.sharedOccurrenceReleased
      ...
      || this.deferredFirstContinuationNextReadInstallPending
      || binding === undefined
      || (!occurrenceOwnerLease
        && !transferredMaterialLease
        && !retiredPredecessorMaterialLease)
      || binding.successorKernel !== this
      || binding.successorFinalCombat === null
      || binding.successorPlan === null
      || binding.successorReservation === null
      || (occurrenceOwnerLease
        ? runInternals.activeOccurrenceId !== null
          || runInternals.pendingReleaseOccurrenceId !== null
        : runInternals.activeOccurrenceId === this.occurrenceId
          || runInternals.pendingReleaseOccurrenceId === this.occurrenceId)
      || runInternals.currentTick120 !== tick120
      || runInternals.pendingFlushTick120 !== tick120
      || before.tick120 !== tick120 - 1
  ...
  private advanceTickUnlocked(input: CanonicalCombatStepInput): CanonicalCombatSnapshot {
    ...
    if (
      ...
      throw new Error("canonical combat deferred install has not been applied");
    }
    ...
    try {
      ...
      const continuationBinding = this.sharedRunState
        ...
        : undefined;
      const successorGameplayDrained = (
        continuationBinding?.phase === "successor-read"
        && continuationBinding.successorKernel === this
        || continuationBinding?.phase === "successor-next-read"
        && continuationBinding.nextSuccessorKernel === this
      ) && snapshot.digitalBodiesDrained;
      if (
        this.sharedRunState
...
interface SuccessorMaterialTransferBoundary {
  readonly binding: Ext013RoomThresholdRunBinding;
  readonly kernel: CanonicalCombatKernel;
  readonly predecessor: CanonicalRoomThresholdMaterialCarryoverSnapshot;
  readonly combat: CanonicalCombatSnapshot;
  readonly runCombat: CanonicalRunCombatStateSnapshot;
}

function requireSuccessorMaterialTransferBoundary(
  runState: CanonicalRunCombatState,
  eventBus: CanonicalEventBus,
  predecessor: CanonicalRoomThresholdMaterialCarryover,
  predecessorOwner: object,
  expectedProposal: PreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer | null,
): SuccessorMaterialTransferBoundary {
  if (!isExactCanonicalRunCombatState(runState)) {
    throw new Error("successor material transfer requires an exact CanonicalRunCombatState");
  }
  if (!isExactCanonicalEventBus(eventBus)) {
    throw new Error("successor material transfer requires the exact canonical event bus");
  }
  const activeProposal = ACTIVE_SUCCESSOR_MATERIAL_TRANSFER_BY_RUN_STATE.get(runState) ?? null;
  if (activeProposal !== expectedProposal) {
    throw new Error("successor material transfer lost its exclusive prepared lease");
  }
  const binding = EXT013_ROOM_THRESHOLD_RUN_BINDINGS.get(runState);
  const predecessorRecord = ROOM_THRESHOLD_MATERIAL_CARRYOVERS.get(predecessor);
  const internals = runCombatStateInternals(runState);
  ...
  if (
    binding?.phase !== "successor-complete"
    || binding.carryover !== predecessor
    || binding.successorOwner !== predecessorOwner
    || binding.successorReservation === null
    || binding.successorPlan === null
    || kernel === null
    || binding.successorFinalCombat === null
    ...
    || predecessorSnapshot.projectiles.length !== 0
    ...
    throw new Error("successor material transfer requires the exact flushed slice-close boundary");
  }
  return Object.freeze({
    binding,
    kernel,
...
/**
 * Reserve the exact post-flush slice-close boundary. Preparation writes no
 * gameplay event and advances neither Run time nor either material stream.
 */
export function prepareCanonicalRunFirstContinuationSuccessorMaterialTransfer(
  predecessorOwner: object,
): PreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer {
  if (typeof predecessorOwner !== "object" || predecessorOwner === null) {
    throw new Error("successor material transfer owner must be opaque");
  }
  const runState = RUN_STATE_BY_FIRST_CONTINUATION_SUCCESSOR_OWNER.get(predecessorOwner);
  const binding = runState === undefined
    ? undefined
    : EXT013_ROOM_THRESHOLD_RUN_BINDINGS.get(runState);
  const predecessor = binding?.carryover ?? null;
  if (runState === undefined || predecessor === null) {
    throw new Error("successor material transfer owner is not registered");
  }
  const eventBus = runCombatStateInternals(runState).bus;
  const boundary = requireSuccessorMaterialTransferBoundary(
    runState,
    eventBus,
    predecessor,
    predecessorOwner,
  ...
  const view = Object.freeze({
    authority:
      "canonical-run-first-continuation-successor-material-transfer-v1" as const,
    extensionPolicy: "EXT-2026-019" as const,
    ...
    predecessorMaterialLease: "retire-on-commit" as const,
  ...
  PREPARED_SUCCESSOR_MATERIAL_TRANSFERS.set(proposal, {
    runState,
    eventBus,
    predecessor,
    predecessorOwner,
    ...
    capturedPredecessorSerialization: JSON.stringify(boundary.predecessor),
  ...
  ACTIVE_SUCCESSOR_MATERIAL_TRANSFER_BY_RUN_STATE.set(runState, proposal);
  return proposal;
}

function requirePreparedSuccessorMaterialTransfer(
  proposal: PreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer,
  expectedStatus: PreparedSuccessorMaterialTransferRecord["status"] = "prepared",
): PreparedSuccessorMaterialTransferRecord {
  if (typeof proposal !== "object" || proposal === null) {
    throw new Error("successor material transfer proposal must be opaque");
  }
  const record = PREPARED_SUCCESSOR_MATERIAL_TRANSFERS.get(proposal);
  if (record === undefined) {
    throw new Error("successor material transfer proposal is not registered");
  }
  if (record.status !== expectedStatus) {
    throw new Error(`successor material transfer proposal is ${record.status}`);
  }
  return record;
}

export function inspectPreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer(
  proposal: PreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer,
): PreparedCanonicalRunFirstContinuationSuccessorMaterialTransferView {
  return requirePreparedSuccessorMaterialTransfer(proposal).view;
}

/**
 * Replace the completed occurrence owner with a material-only owner without a
 * binding gap. The original projectile authority is retained; no projectile is
 * copied, recreated, or reclaimed.
 */
export function commitPreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer(
  proposal: PreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer,
): CanonicalRunOccurrenceMaterialCarryover {
  const record = requirePreparedSuccessorMaterialTransfer(proposal);
  try {
    const boundary = requireSuccessorMaterialTransferBoundary(
      record.runState,
      record.eventBus,
    ...
    if (
      ...
      || JSON.stringify(boundary.predecessor) !== record.capturedPredecessorSerialization
      ...
      throw new Error("prepared successor material transfer became stale");
    }
    const carryover = new CanonicalRunOccurrenceMaterialCarryover(
      record.runState,
      record.eventBus,
      record.predecessor,
      record.predecessorOwner,
      ...
      CREATE_RUN_OCCURRENCE_MATERIAL_CARRYOVER,
    );
    boundary.binding.phase = "successor-material";
    boundary.binding.successorMaterial = carryover;
    boundary.binding.predecessorMaterialLeaseRetiredAtTick120 = boundary.combat.tick120;
    record.status = "committed";
    ACTIVE_SUCCESSOR_MATERIAL_TRANSFER_BY_RUN_STATE.delete(record.runState);
    return carryover;
  } catch (error) {
    record.status = "failed";
    ACTIVE_SUCCESSOR_MATERIAL_TRANSFER_BY_RUN_STATE.delete(record.runState);
...
export function cancelPreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer(
  proposal: PreparedCanonicalRunFirstContinuationSuccessorMaterialTransfer,
): void {
  const record = requirePreparedSuccessorMaterialTransfer(proposal);
  if (ACTIVE_SUCCESSOR_MATERIAL_TRANSFER_BY_RUN_STATE.get(record.runState) !== proposal) {
    throw new Error("successor material transfer lost its exclusive prepared lease");
  }
...
/** Opaque collisionless material owner for one released Run occurrence. */
export class CanonicalRunOccurrenceMaterialCarryover {
  constructor(
    runState: CanonicalRunCombatState,
    eventBus: CanonicalEventBus,
    predecessor: CanonicalRoomThresholdMaterialCarryover,
    ...
    if (creationToken !== CREATE_RUN_OCCURRENCE_MATERIAL_CARRYOVER) {
      throw new Error("Run occurrence material carryover requires a committed transfer");
    }
    RUN_OCCURRENCE_MATERIAL_CARRYOVERS.set(this, {
      runState,
      eventBus,
      predecessor,
      predecessorOwner,
  ...
  snapshot(): CanonicalRunOccurrenceMaterialCarryoverSnapshot {
    const record = RUN_OCCURRENCE_MATERIAL_CARRYOVERS.get(this);
    ...
    if (
      ...
      || combat.projectiles.some((projectile) =>
        projectile.state !== "residue" || projectile.collisionEnabled)
      || combat.tick120 !== record.runState.snapshot().tick120
    ) {
      throw new Error("Run occurrence material carryover acquired gameplay-capable state");
    }
    return Object.freeze({
      authority: "canonical-run-occurrence-material-carryover-v1" as const,
      extensionPolicy: "EXT-2026-019" as const,
      ...
      predecessorMaterialLease: "retired" as const,
      gameplayAuthority: "released" as const,
      roomCompletion: "withheld" as const,
      roomHandoff: "withheld" as const,
      nextOccurrenceAdmission:
...
export function inspectCanonicalRunFirstContinuationNextOccurrenceDormantOwner(
  ...
  const projectilePoolAudit = binding.nextSuccessorKernel?.projectilePoolAudit()
    ?? Object.freeze([]);
  const predecessor = binding.carryover?.snapshot() ?? null;
  const claimedCount = runCombat.claimedOccurrenceIds.filter((occurrenceId) =>
    occurrenceId === plan.occurrence.occurrenceId).length;
  const phase = binding.phase === "successor-next-dormant"
    ? "dormant" as const
    : binding.phase === "successor-next-pre-read"
      ? "pre-read" as const
      : binding.phase === "successor-next-read"
  ...
  if (
    ...
    || (phase === "dormant"
      ...
      : phase === "pre-read"
        ? relativeTick120 < 1
          || runCombat.tick120 >= boundaries.readStartTick120
          || combat !== null
          || claimedCount !== 0
        : phase === "read"
          ? runCombat.tick120 < boundaries.readStartTick120
    ...
    || (phase === "read"
      ...
      : runCombat.activeOccurrenceId !== null)
    || runCombat.pendingFlushTick120 !== null
    || material.tick120 !== runCombat.tick120
    ...
    || DORMANT_SUCCESSOR_POOL_CLASS_ORDER.some((poolClass) =>
      material.poolUsage.allocatedSlots[poolClass]
        !== evaluation.carryover.allocatedSlots[poolClass])
    || predecessor === null
    || predecessor.tick120 !== runCombat.tick120
    || !predecessor.drained
    || predecessor.materialCount !== 0
    || predecessor.poolUsage.liveColliders !== 0
    || room.tick120 !== runCombat.tick120
    || room.state !== "idle"
    || room.currentRoom !== plan.targetRoom
    || room.targetRoom !== null
    || room.active !== null
    || player.tick120 !== runCombat.tick120
    || override.state !== "idle"
    || override.deadlineTick120 !== null
    || override.localVoid !== null
    || (phase === "dormant" || phase === "pre-read"
      ? player.state !== "alive"
        || !player.collisionEnabled
        || player.activeLeases.length !== 0
      : combat === null
        || combat.tick120 !== runCombat.tick120
        || combat.relativeTick120 !== runCombat.tick120 - boundaries.readStartTick120
        || combat.patternId !== plan.occurrence.patternId
        || combat.occurrenceId !== plan.occurrence.occurrenceId
        || combat.playerPosition.x !== runCombat.playerPosition.x
        || combat.playerPosition.y !== runCombat.playerPosition.y
        || combat.player.tick120 !== runCombat.tick120
        || combat.player.state !== player.state
        || combat.player.collisionEnabled !== player.collisionEnabled
        || combat.override.state !== override.state
        || combat.override.deadlineTick120 !== override.deadlineTick120
    ...
    throw new Error(
      "next occurrence owner left its exact dormant/pre-READ/READ/release boundary",
    );
  }
  ...
  return Object.freeze({
    ...
    relativeTick120,
    boundaryTicks120: boundaries,
    telegraphStartTick120: boundaries.telegraphStartTick120,
...
export interface CanonicalRunFirstContinuationNextOccurrencePreReadTickResult {
  readonly runCombat: CanonicalRunCombatStateSnapshot;
  readonly material: CanonicalRunOccurrenceMaterialCarryoverSnapshot;
...
export interface CanonicalRunFirstContinuationNextOccurrenceReadStartResult {
  readonly runCombat: CanonicalRunCombatStateSnapshot;
  readonly material: CanonicalRunOccurrenceMaterialCarryoverSnapshot;
  readonly combat: CanonicalCombatSnapshot;
  readonly flushedEvents: readonly CanonicalGameplayEvent[];
...
export interface CanonicalRunFirstContinuationNextOccurrenceReadTickResult {
  readonly runCombat: CanonicalRunCombatStateSnapshot;
  readonly material: CanonicalRunOccurrenceMaterialCarryoverSnapshot;
  readonly combat: CanonicalCombatSnapshot;
  readonly flushedEvents: readonly CanonicalGameplayEvent[];
...
interface NextOccurrenceExecutionBoundary {
  readonly runState: CanonicalRunCombatState;
  readonly binding: Ext013RoomThresholdRunBinding;
  readonly internals: RunCombatStateInternals;
  readonly materialOwner: CanonicalRunOccurrenceMaterialCarryover;
  readonly materialRecord: RunOccurrenceMaterialCarryoverRecord;
  readonly predecessor: CanonicalRoomThresholdMaterialCarryover;
  readonly predecessorRecord: RoomThresholdMaterialCarryoverRecord;
  readonly plan: CanonicalRunFirstContinuationNextOccurrencePlanPayload;
  readonly evaluation: CanonicalRunFirstContinuationCombinedPoolAdmissionEvaluation;
  readonly boundaries: Readonly<CanonicalRunFirstContinuationSuccessorBoundaryTicks>;
}

function requireNextOccurrenceExecutionBoundary(
  owner: CanonicalRunFirstContinuationNextOccurrenceDormantOwner,
): NextOccurrenceExecutionBoundary {
  const runState = RUN_STATE_BY_NEXT_OCCURRENCE_OWNER.get(owner);
  const binding = runState === undefined
    ? undefined
    : EXT013_ROOM_THRESHOLD_RUN_BINDINGS.get(runState);
  ...
  if (
    ...
    throw new Error("next occurrence execution lost its exact dormant/pre-READ owner lease");
  }
  inspectCanonicalRunFirstContinuationNextOccurrenceDormantOwner(owner);
  return Object.freeze({
    runState,
    binding,
    internals: runCombatStateInternals(runState),
    materialOwner,
    materialRecord,
    predecessor,
    predecessorRecord,
    plan: binding.nextSuccessorPlan,
    evaluation: binding.nextSuccessorAdmission,
    boundaries: successorBoundaryTicks(binding.nextSuccessorPlan),
  });
}

function advanceNextOccurrenceCollisionlessBoundary(
  boundary: NextOccurrenceExecutionBoundary,
  input: CanonicalCombatStepInput,
  latestTick120: number,
  label: "next occurrence pre-READ" | "next occurrence READ start",
): Readonly<{
  runCombat: CanonicalRunCombatStateSnapshot;
  material: CanonicalRunOccurrenceMaterialCarryoverSnapshot;
  flushedEvents: readonly CanonicalGameplayEvent[];
}> {
  const {
    runState,
    binding,
  ...
  try {
    ...
    if (flushedEvents.some((event) =>
      ...
      throw new Error(`${label} emitted a non-material event`);
    }
    const runCombat = runState.snapshot();
    const material = materialOwner.snapshot();
    if (
      runCombat.tick120 !== advanced.runCombat.tick120
      || runCombat.pendingFlushTick120 !== null
      || runCombat.activeOccurrenceId !== null
      || internals.bus.pendingEventCount() !== 0
      || material.tick120 !== runCombat.tick120
      ...
      || DORMANT_SUCCESSOR_POOL_CLASS_ORDER.some((poolClass) =>
        material.poolUsage.allocatedSlots[poolClass]
          !== boundary.evaluation.carryover.allocatedSlots[poolClass])
    ) {
      throw new Error(`${label} did not close its exact sole-flush material tick`);
    }
    return Object.freeze({runCombat, material, flushedEvents});
  } catch (error) {
...
/** Advance exactly one telegraph/entry tick; READ local tick zero remains separate. */
export function advanceCanonicalRunFirstContinuationNextOccurrencePreReadTick(
  owner: CanonicalRunFirstContinuationNextOccurrenceDormantOwner,
  input: CanonicalCombatStepInput,
): CanonicalRunFirstContinuationNextOccurrencePreReadTickResult {
  const boundary = requireNextOccurrenceExecutionBoundary(owner);
  if (boundary.internals.currentTick120 >= boundary.boundaries.readStartTick120 - 1) {
    throw new Error("next occurrence pre-READ stops before the exact READ claim tick");
  }
  const advanced = advanceNextOccurrenceCollisionlessBoundary(
    boundary,
    input,
    boundary.boundaries.readStartTick120 - 1,
    "next occurrence pre-READ",
  );
  const snapshot = inspectCanonicalRunFirstContinuationNextOccurrenceDormantOwner(owner);
  if (
    snapshot.phase !== "pre-read"
    || snapshot.tick120 !== advanced.runCombat.tick120
    || snapshot.combat !== null
  ) {
    const error = new Error("next occurrence pre-READ lost its one-tick authored phase");
    if (boundary.internals.fault === null) boundary.internals.fault = error;
    throw error;
  }
  return Object.freeze({...advanced, combat: null});
}

/** Close T+159, sole-flush retained material, then claim and install READ local tick zero. */
export function startCanonicalRunFirstContinuationNextOccurrenceReadBinding(
  owner: CanonicalRunFirstContinuationNextOccurrenceDormantOwner,
  input: CanonicalCombatStepInput,
): CanonicalRunFirstContinuationNextOccurrenceReadStartResult {
  const boundary = requireNextOccurrenceExecutionBoundary(owner);
  const {runState, binding, internals, plan, evaluation, boundaries} = boundary;
  if (
    binding.phase !== "successor-next-pre-read"
    || internals.currentTick120 !== boundaries.readStartTick120 - 1
  ) {
    throw new Error("next occurrence READ start requires the exact T+158 entry boundary");
  }
  const validated = validateCombatStepAgainstRunState(input, internals);
  if (
    validated.tick120 !== boundaries.readStartTick120
    || validated.overridePressed
    || validated.overrideReleased
  ) {
    throw new Error("next occurrence READ start requires exact T+159 input with Override locked");
  }
  ...
  const kernel = new CanonicalCombatKernel({
    ...
    startTick120: boundaries.readStartTick120,
  ...
  }, runState, DEFERRED_FIRST_CONTINUATION_NEXT_READ_INSTALL,
  nextOccurrenceProjectilePoolBudgets(evaluation));
  let authoritativeTickAccepted = false;
  try {
    const advanced = advanceNextOccurrenceCollisionlessBoundary(
      boundary,
      input,
      boundaries.readStartTick120,
      "next occurrence READ start",
    );
    authoritativeTickAccepted = true;
    kernel[DEFERRED_FIRST_CONTINUATION_NEXT_READ_INSTALL](
      runState,
      owner,
      plan,
      evaluation,
    ...
    if (
      snapshot.phase !== "read"
      || snapshot.authoredPhase !== "read"
      || snapshot.tick120 !== boundaries.readStartTick120
      || snapshot.relativeTick120 !== 159
      || snapshot.nextMasterTickAction !== "advance-read"
      || snapshot.combat === null
      || snapshot.combat.relativeTick120 !== 0
      || snapshot.combat.projectiles.length !== 0
      || snapshot.combat.rngCallsConsumed !== 0
      || snapshot.runCombat.activeOccurrenceId !== plan.occurrence.occurrenceId
    ) {
      throw new Error("next occurrence READ start did not install its empty local-tick-zero kernel");
    }
    return Object.freeze({
      runCombat: snapshot.runCombat,
      material: snapshot.material,
      combat: snapshot.combat,
      flushedEvents: advanced.flushedEvents,
    });
  } catch (error) {
    if (authoritativeTickAccepted && internals.fault === null) {
      internals.fault = error instanceof Error ? error : new Error(String(error));
...
interface NextOccurrenceReadExecutionBoundary extends NextOccurrenceExecutionBoundary {
  readonly nextKernel: CanonicalCombatKernel;
}

function requireNextOccurrenceReadExecutionBoundary(
  owner: CanonicalRunFirstContinuationNextOccurrenceDormantOwner,
): NextOccurrenceReadExecutionBoundary {
  const runState = RUN_STATE_BY_NEXT_OCCURRENCE_OWNER.get(owner);
  const binding = runState === undefined
    ? undefined
    : EXT013_ROOM_THRESHOLD_RUN_BINDINGS.get(runState);
  ...
  const predecessorRecord = predecessor === null
    ? undefined
    : ROOM_THRESHOLD_MATERIAL_CARRYOVERS.get(predecessor);
  const nextKernel = binding?.nextSuccessorKernel ?? null;
  if (
    runState === undefined
    || binding?.phase !== "successor-next-read"
    || binding.nextSuccessorOwner !== owner
    ...
    throw new Error("next occurrence READ lost its exact active owner lease");
  }
  const snapshot = inspectCanonicalRunFirstContinuationNextOccurrenceDormantOwner(owner);
  if (
    snapshot.phase !== "read"
    ...
    throw new Error("next occurrence READ requires its exact flushed active boundary");
  }
  return Object.freeze({
    runState,
    binding,
...
/**
 * Advance the second occurrence combat, retained first-occurrence material,
 * drained transition lineage, idle room, shared player, and sole Run flush as
 * one exact READ tick. Gameplay releases before collisionless residue drains.
 */
export function advanceCanonicalRunFirstContinuationNextOccurrenceReadTick(
  owner: CanonicalRunFirstContinuationNextOccurrenceDormantOwner,
  input: CanonicalCombatStepInput,
): CanonicalRunFirstContinuationNextOccurrenceReadTickResult {
  const boundary = requireNextOccurrenceReadExecutionBoundary(owner);
  const {
    runState,
    binding,
    internals,
  ...
  if (internals.advanceLocked) {
    throw new Error("next occurrence READ advance is already in progress");
  }
  assertRunCombatStateOperational(internals);
  if (
    internals.pendingFlushTick120 !== null
    ...
    throw new Error("next occurrence READ requires a flushed active occurrence boundary");
  }
  const validated = validateCombatStepAgainstRunState(input, internals);
  if (validated.overridePressed || validated.overrideReleased) {
    throw new Error("next occurrence READ cannot admit an Override edge; Override locked");
  }
  if (validated.tick120 > boundaries.materialSettleStartTick120) {
    throw new Error("next occurrence READ stops at the exact gameplay release boundary");
  }
  const beforeMaterial = materialOwner.snapshot();
  const beforePredecessor = predecessor.snapshot();
  const beforeSourceCombat = binding.successorFinalCombat;
  const beforeCombat = CanonicalCombatKernel.prototype.snapshot.call(nextKernel);
  const beforeRoom = RoomTransitionAuthority.prototype.snapshot.call(binding.roomTransition);
  if (
    beforeSourceCombat === null
    || beforeCombat.tick120 !== internals.currentTick120
    || beforeCombat.patternId !== plan.occurrence.patternId
    || beforeCombat.occurrenceId !== plan.occurrence.occurrenceId
    ...
    || beforeMaterial.projectiles.some((projectile) =>
      projectile.state !== "residue" || projectile.collisionEnabled)
    || !nextOccurrencePoolUsageFitsAdmission(beforeMaterial, beforeCombat, evaluation)
    || beforePredecessor.tick120 !== internals.currentTick120
    || !beforePredecessor.drained
    || beforePredecessor.materialCount !== 0
    || beforePredecessor.poolUsage.liveColliders !== 0
    || beforeSourceCombat.tick120 !== internals.currentTick120
    || !beforeSourceCombat.patternComplete
    || !beforeSourceCombat.digitalBodiesDrained
    || beforeSourceCombat.poolUsage.liveColliders !== 0
    || beforeSourceCombat.projectiles.some((projectile) =>
      projectile.state !== "residue" || projectile.collisionEnabled)
    || binding.materialRoomEventCount === null
    || beforeRoom.tick120 !== internals.currentTick120
    || beforeRoom.state !== "idle"
    || beforeRoom.currentRoom !== plan.targetRoom
    ...
    throw new Error("next occurrence READ authorities lost their sealed pre-tick boundary");
  }
  const roomProposal = RoomTransitionAuthority.prototype.prepareAdvance.call(
    binding.roomTransition,
    validated.tick120,
  ...
  if (
    ...
    || roomView.preview.currentRoom !== plan.targetRoom
    ...
    throw new Error("next occurrence READ target-room FSM proposal drifted");
  }

  internals.advanceLocked = true;
  let authoritativeTickAccepted = false;
  try {
    const combat = nextKernel[SEALED_FIRST_CONTINUATION_NEXT_READ_ADVANCE](owner, input);
    authoritativeTickAccepted = true;
    const combatPendingEventCount = internals.bus.pendingEventCount();
    predecessorRecord.projectiles.advanceTo(validated.tick120);
    predecessorRecord.currentTick120 = validated.tick120;
    const sourceCombat = materialRecord.kernel[SEALED_FIRST_CONTINUATION_MATERIAL_ADVANCE](
      materialOwner,
      validated.tick120,
    );
    ...
    const material = materialOwner.snapshot();
    const predecessorAfter = predecessor.snapshot();
    const room = RoomTransitionAuthority.prototype.snapshot.call(binding.roomTransition);
    ...
    if (
      ...
      || internals.activeOccurrenceId !== plan.occurrence.occurrenceId
      || (releasesOccurrence
        ? validated.tick120 !== boundaries.materialSettleStartTick120
          || internals.pendingReleaseOccurrenceId !== plan.occurrence.occurrenceId
          || !combat.patternComplete
          || combat.poolUsage.liveColliders !== 0
          || combat.projectiles.some((projectile) =>
      ...
      || material.projectiles.some((projectile) =>
        projectile.state !== "residue" || projectile.collisionEnabled)
      || !nextOccurrencePoolUsageFitsAdmission(material, combat, evaluation)
      || predecessorAfter.tick120 !== validated.tick120
      || !predecessorAfter.drained
      || predecessorAfter.materialCount !== 0
      || predecessorAfter.poolUsage.liveColliders !== 0
      || sourceCombat.tick120 !== validated.tick120
      || !sourceCombat.patternComplete
      || !sourceCombat.digitalBodiesDrained
      || sourceCombat.poolUsage.liveColliders !== 0
      || sourceCombat.projectiles.some((projectile) =>
      ...
      || room.eventCount !== binding.materialRoomEventCount
      || room.active !== null
      || internals.bus.pendingEventCount() !== combatPendingEventCount
    ) {
      throw new Error("next occurrence READ tick lost combat/material/room synchronization");
    }
    if (releasesOccurrence) binding.phase = "successor-next-release-requested";
    ...
    if (
      ...
      throw new Error("next occurrence READ tick did not close its sole Run-owned flush");
    }
...
/**
 * Advance the transferred second-occurrence residue together with both older
 * drained lineages, the idle room, and the shared Run clock. Draining the
 * residue does not release its allocation or authorize another occurrence.
 */
export function advanceCanonicalRunFirstContinuationNextOccurrenceMaterialHold(
  materialOwner: CanonicalRunFirstContinuationNextOccurrenceMaterialCarryover,
  input: CanonicalCombatStepInput,
): CanonicalRunFirstContinuationNextOccurrenceMaterialHoldResult {
  const record = NEXT_OCCURRENCE_MATERIAL_CARRYOVERS.get(materialOwner);
  if (record === undefined) {
    throw new Error("next occurrence material hold requires an exact material owner");
  }
  const {runState, eventBus} = record;
  if (!isExactCanonicalRunCombatState(runState)) {
    throw new Error("next occurrence material hold requires an exact CanonicalRunCombatState");
  }
  if (!isExactCanonicalEventBus(eventBus)) {
    throw new Error("next occurrence material hold requires the exact canonical event bus");
  }
  const binding = EXT013_ROOM_THRESHOLD_RUN_BINDINGS.get(runState);
  const predecessorRecord = RUN_OCCURRENCE_MATERIAL_CARRYOVERS.get(record.predecessor);
  const transitionRecord = ROOM_THRESHOLD_MATERIAL_CARRYOVERS.get(
    record.transitionPredecessor,
  );
  const internals = runCombatStateInternals(runState);
  if (
    ...
    || binding.nextSuccessorOwner !== record.predecessorOwner
    || binding.successorMaterial !== record.predecessor
    || binding.successorKernel !== record.predecessorRecord.kernel
    || binding.successorFinalCombat === null
    || binding.carryover !== record.transitionPredecessor
    || predecessorRecord !== record.predecessorRecord
    || predecessorRecord.runState !== runState
    || predecessorRecord.eventBus !== eventBus
    || predecessorRecord.predecessor !== record.transitionPredecessor
    || transitionRecord?.runState !== runState
    || binding.predecessorMaterialLeaseRetiredAtTick120 === null
    || binding.successorMaterialCapacityLeaseRetiredAtTick120
      !== record.transferredAtTick120
    ...
    || binding.expectedPendingEventCount !== null
    || ACTIVE_SUCCESSOR_MATERIAL_TRANSFER_BY_RUN_STATE.has(runState)
    || ACTIVE_NEXT_OCCURRENCE_ADMISSION_BY_MATERIAL.has(record.predecessor)
    || ACTIVE_NEXT_OCCURRENCE_MATERIAL_TRANSFER_BY_RUN_STATE.has(runState)
  ) {
    throw new Error("next occurrence material hold lost its exact transferred owner lease");
  }

  const beforeRun = runState.snapshot();
  const beforeMaterial = materialOwner.snapshot();
  const beforeSourceCombat = CanonicalCombatKernel.prototype.snapshot.call(
    record.predecessorRecord.kernel,
  );
  const beforeCombat = CanonicalCombatKernel.prototype.snapshot.call(record.kernel);
  const beforeTransition = record.transitionPredecessor.snapshot();
  const beforeRoom = RoomTransitionAuthority.prototype.snapshot.call(
    binding.roomTransition,
  );
  const nextTick120 = requireSafeTick(
    beforeRun.tick120 + 1,
    "next occurrence material hold next tick",
  );
  if (
    beforeRun.tick120 !== internals.currentTick120
    || beforeRun.activeOccurrenceId !== null
    || beforeRun.pendingFlushTick120 !== null
    || beforeMaterial.tick120 !== beforeRun.tick120
    || beforeMaterial.sourcePatternId !== record.sourcePatternId
    || beforeMaterial.sourceOccurrenceId !== record.sourceOccurrenceId
    || beforeMaterial.poolUsage.liveColliders !== 0
    || beforeMaterial.projectiles.some((projectile) =>
      projectile.state !== "residue" || projectile.collisionEnabled)
    || beforeCombat.tick120 !== beforeRun.tick120
    || beforeCombat.patternId !== record.sourcePatternId
    || beforeCombat.occurrenceId !== record.sourceOccurrenceId
    || !beforeCombat.patternComplete
    || !beforeCombat.digitalBodiesDrained
    || beforeCombat.poolUsage.liveColliders !== 0
    || beforeSourceCombat.tick120 !== beforeRun.tick120
    || !beforeSourceCombat.patternComplete
    || !beforeSourceCombat.digitalBodiesDrained
    || beforeSourceCombat.projectiles.length !== 0
    || beforeSourceCombat.poolUsage.liveColliders !== 0
    || beforeTransition.tick120 !== beforeRun.tick120
    || !beforeTransition.drained
    || beforeTransition.materialCount !== 0
    || beforeTransition.poolUsage.liveColliders !== 0
    || binding.nextSuccessorFinalCombat.tick120 !== beforeRun.tick120
    || binding.successorFinalCombat.tick120 !== beforeRun.tick120
    || beforeRoom.tick120 !== beforeRun.tick120
    || beforeRoom.state !== "idle"
    || beforeRoom.currentRoom !== binding.targetRoom
    || beforeRoom.targetRoom !== null
    || beforeRoom.generation !== 1
    ...
    throw new Error("next occurrence material hold lost its sealed pre-tick boundary");
  }
  ...
  try {
    const advanced = advanceCanonicalRunIdleWithContinuationMaterial(
      ...
      transitionRecord,
      Object.freeze({
        label: "next occurrence transferred material hold" as const,
        requireDrained: true,
        requireOverrideQuiescent: true,
        latestTick120: null,
      }),
    ...
    authoritativeTickAccepted = true;
    const sourceCombat = record.predecessorRecord.kernel[
      SEALED_FIRST_CONTINUATION_MATERIAL_ADVANCE
    ](record.predecessor, advanced.runCombat.tick120);
    binding.successorFinalCombat = sourceCombat;
    const combat = record.kernel[SEALED_FIRST_CONTINUATION_NEXT_MATERIAL_ADVANCE](
      materialOwner,
      advanced.runCombat.tick120,
    );
    binding.nextSuccessorFinalCombat = combat;
    binding.expectedPendingEventCount = eventBus.pendingEventCount();
    const flushedEvents = runState.flushTick(advanced.runCombat.tick120);
    if (flushedEvents.some((event) => !isNextOccurrenceTailEventAllowed(event.id))) {
      throw new Error("next occurrence material hold emitted an unowned gameplay event");
    }
    ...
    const transition = record.transitionPredecessor.snapshot();
    const room = RoomTransitionAuthority.prototype.snapshot.call(binding.roomTransition);
    const retainedProjectileIdentities = new Set(beforeMaterial.projectiles.map(
      (projectile) => `${projectile.instanceId}:${projectile.generation}`,
    ));
    if (
      runCombat.tick120 !== nextTick120
      || runCombat.pendingFlushTick120 !== null
      || runCombat.activeOccurrenceId !== null
      || eventBus.pendingEventCount() !== 0
      || runCombat.claimedOccurrenceIds.length !== beforeRun.claimedOccurrenceIds.length
      || runCombat.claimedOccurrenceIds.some((occurrenceId, index) =>
        occurrenceId !== beforeRun.claimedOccurrenceIds[index])
      || material.tick120 !== nextTick120
      || material.sourcePatternId !== record.sourcePatternId
      || material.sourceOccurrenceId !== record.sourceOccurrenceId
      || material.materialCount !== combat.projectiles.length
      || material.rngCallsConsumed !== beforeMaterial.rngCallsConsumed
      || material.poolUsage.liveColliders !== 0
      || DORMANT_SUCCESSOR_POOL_CLASS_ORDER.some((poolClass) =>
        material.poolUsage.allocatedSlots[poolClass]
          !== beforeMaterial.poolUsage.allocatedSlots[poolClass])
      || material.projectiles.some((projectile) =>
        projectile.state !== "residue"
        || projectile.collisionEnabled
        || !retainedProjectileIdentities.has(
          `${projectile.instanceId}:${projectile.generation}`,
        ))
      || combat.tick120 !== nextTick120
      || combat.rngCallsConsumed !== beforeCombat.rngCallsConsumed
      || !combat.patternComplete
      || !combat.digitalBodiesDrained
      || combat.poolUsage.liveColliders !== 0
      || sourceCombat.tick120 !== nextTick120
      || sourceCombat.rngCallsConsumed !== beforeSourceCombat.rngCallsConsumed
      || !sourceCombat.patternComplete
      || !sourceCombat.digitalBodiesDrained
      || sourceCombat.projectiles.length !== 0
      || sourceCombat.poolUsage.liveColliders !== 0
      || transition.tick120 !== nextTick120
      || !transition.drained
      || transition.materialCount !== 0
      || transition.poolUsage.liveColliders !== 0
      || room.tick120 !== nextTick120
      ...
      || room.eventCount !== binding.materialRoomEventCount
      || room.active !== null
      || material.predecessorOccurrenceMaterialLease !== "retired"
      || material.transitionMaterialLease !== "retired"
      || material.gameplayAuthority !== "released"
      || material.roomCompletion !== "withheld"
      || material.roomHandoff !== "withheld"
      || material.nextOccurrenceAdmission !== "withheld-pending-decision"
    ) {
      throw new Error("next occurrence material hold did not close its exact four-lineage tick");
    }
    return Object.freeze({runCombat, material, flushedEvents});
  } catch (error) {
    if (authoritativeTickAccepted && internals.fault === null) {
...
interface ContinuationMaterialIdleAdvancePolicy {
  readonly label:
    ...
    | "next occurrence transferred material hold";
...
function advanceCanonicalRunIdleWithContinuationMaterial(
  ...
  try {
    ...
    if (validated.overridePressed || validated.overrideReleased) {
      throw new Error(
        `${policy.label} cannot admit an Override edge before successor local resistance; Override locked`,
      );
    }
...
/**
 * Temporary idle coordinator for the transferred material boundary. A later
 * occurrence coordinator may replace this idle port, but it must retain the
 * same sole-flush and collisionless-material ordering.
 */
export function advanceCanonicalRunFirstContinuationSuccessorMaterialHold(
  materialOwner: CanonicalRunOccurrenceMaterialCarryover,
  input: CanonicalCombatStepInput,
): CanonicalRunFirstContinuationSuccessorMaterialHoldResult {
  const record = RUN_OCCURRENCE_MATERIAL_CARRYOVERS.get(materialOwner);
  if (record === undefined) {
    throw new Error("successor material hold requires an exact material owner");
  }
  const {runState, eventBus} = record;
  if (!isExactCanonicalRunCombatState(runState)) {
    throw new Error("successor material hold requires an exact CanonicalRunCombatState");
  }
  if (!isExactCanonicalEventBus(eventBus)) {
    throw new Error("successor material hold requires the exact canonical event bus");
  }
  const binding = EXT013_ROOM_THRESHOLD_RUN_BINDINGS.get(runState);
  const predecessorRecord = ROOM_THRESHOLD_MATERIAL_CARRYOVERS.get(record.predecessor);
  const internals = runCombatStateInternals(runState);
  if (
    ...
    throw new Error("successor material hold lost its exact transferred owner lease");
  }
  let authoritativeTickAccepted = false;
  try {
    const advanced = advanceCanonicalRunIdleWithContinuationMaterial(
      ...
      predecessorRecord,
      Object.freeze({
        label: "successor transferred material hold" as const,
        requireDrained: true,
        requireOverrideQuiescent: true,
        latestTick120: null,
      }),
    ...
    authoritativeTickAccepted = true;
    const materialCombat = record.kernel[SEALED_FIRST_CONTINUATION_MATERIAL_ADVANCE](
      materialOwner,
      advanced.runCombat.tick120,
    );
    binding.successorFinalCombat = materialCombat;
    binding.expectedPendingEventCount = internals.bus.pendingEventCount();
    const flushedEvents = runState.flushTick(advanced.runCombat.tick120);
    const runCombat = runState.snapshot();
    const material = materialOwner.snapshot();
    if (
      runCombat.tick120 !== material.tick120
      || runCombat.pendingFlushTick120 !== null
      || runCombat.activeOccurrenceId !== null
      || eventBus.pendingEventCount() !== 0
      || material.sourcePatternId !== materialCombat.patternId
      || material.sourceOccurrenceId !== materialCombat.occurrenceId
      || material.materialCount !== materialCombat.projectiles.length
      || material.poolUsage.liveColliders !== 0
      || material.projectiles.some((projectile) =>
        projectile.state !== "residue" || projectile.collisionEnabled)
      || flushedEvents.some((event) =>
        event.id !== "projectile.residue.remove"
        && event.id !== "projectile.lifecycle.complete")
    ) {
      throw new Error("successor material hold did not close its exact material-only tick");
    }
    return Object.freeze({runCombat, material, flushedEvents});
  } catch (error) {
    if (authoritativeTickAccepted && internals.fault === null) {
...
function nextOccurrenceProjectilePoolBudgets(
  evaluation: CanonicalRunFirstContinuationCombinedPoolAdmissionEvaluation,
): ProjectilePoolBudgets {
  const resolution = evaluation.poolClassResolution;
  const reservation = evaluation.successor.reservationByClass;
  if (
    evaluation.state !== "admissible"
    || !evaluation.admissible
    ...
    throw new Error("next occurrence projectile budgets require committed admissible evaluation");
  }
  const requestedProjectileSlots = requirePositiveInteger(
    evaluation.successor.requestedProjectileSlots,
    "next occurrence requested projectile slots",
  );
  const requestedResidueVisualSlots = requirePositiveInteger(
    evaluation.successor.requestedResidueVisualSlots,
    "next occurrence requested residue visual slots",
  );
  if (DORMANT_SUCCESSOR_POOL_CLASS_ORDER.some((poolClass) =>
    reservation[poolClass]
      !== (poolClass === resolution.poolClass ? requestedProjectileSlots : 0))) {
    throw new Error("next occurrence reservation no longer matches its resolved pool class");
  }
  return Object.freeze({
    micro: reservation.micro,
    medium: reservation.medium,
    heavy: reservation.heavy,
    splitChildren: reservation.splitChildren,
    residueVisualOnly: requestedResidueVisualSlots,
  });
}

function nextOccurrencePoolUsagesFitAdmission(
  material: ProjectilePoolUsage,
  combat: ProjectilePoolUsage,
  evaluation: CanonicalRunFirstContinuationCombinedPoolAdmissionEvaluation,
): boolean {
  const reservation = evaluation.successor.reservationByClass;
  ...
  return DORMANT_SUCCESSOR_POOL_CLASS_ORDER.every((poolClass) =>
    material.allocatedSlots[poolClass]
        === evaluation.carryover.allocatedSlots[poolClass]
      && combat.active[poolClass] <= reservation[poolClass]
      && combat.allocatedSlots[poolClass] <= reservation[poolClass]
      && material.allocatedSlots[poolClass]
        + combat.allocatedSlots[poolClass]
        <= combined.allocatedSlots[poolClass])
    && material.residueVisuals <= evaluation.carryover.residueVisuals
    && combat.residueVisuals
      <= evaluation.successor.requestedResidueVisualSlots
    && material.residueVisuals + combat.residueVisuals
      <= combined.residueVisuals;
}

function nextOccurrencePoolUsageFitsAdmission(
  material: CanonicalRunOccurrenceMaterialCarryoverSnapshot,
  combat: CanonicalCombatSnapshot,
  evaluation: CanonicalRunFirstContinuationCombinedPoolAdmissionEvaluation,
): boolean {
  return nextOccurrencePoolUsagesFitAdmission(
    material.poolUsage,
    combat.poolUsage,
    evaluation,
  );
...
export function commitPreparedCanonicalRunFirstContinuationDormantSuccessorTransfer(
  ...
  try {
    ...
    RUN_STATE_BY_FIRST_CONTINUATION_SUCCESSOR_OWNER.set(
      record.successorOwner,
      record.runState,
    );
    record.status = "committed";
