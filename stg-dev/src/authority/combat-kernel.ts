...
export interface CanonicalMisreaderEnforceEntryOptions {
  /** One-use occurrence identity; this fragment does not infer room or scheduler ordinals. */
  readonly occurrenceId: string;
  ...
  /** Player authority handoff position at `phaseEntryTick120 - 1`. */
  readonly initialPlayerPosition: Vec2;
  /** Adapter gap: V4 laser lifecycle does not declare a universal contact damage amount. */
  readonly projectileDamage: number;
}
...
function captureMisreaderEnforceEntryOptions(
  ...
  return Object.freeze({
    ...
    initialPlayerPosition: freezeVec2(
      ...
      "Misreader enforce-entry initialPlayerPosition",
    ),
    projectileDamage: requirePositiveInteger(
      captured.projectileDamage as number,
      "Misreader enforce-entry projectileDamage",
    ),
  });
}
...
export class CanonicalMisreaderEnforceEntryFragment {
  ...
  private pendingFlushTick120: number | null = null;
  ...
  constructor(
    ...
    if (!isExactCanonicalEventBus(eventBus)) {
      throw new Error("Misreader fragment requires an exact CanonicalEventBus instance");
    }
    if (!isExactBossPhaseAuthority(boss)) {
    ...
    if (options.phaseEntryTick120 < 1) {
      throw new Error("Misreader enforce entry must be a positive master tick");
    }
    const startTick120 = options.phaseEntryTick120 - 1;
    ...
    if (
      ...
      throw new Error("Misreader initialPlayerPosition must remain inside the logical viewport");
    }
    ...
    // Prove the already-active Boss and the new laser share one event bus and
    // exact manifest binding before the fragment owns any timed authority.
    const bossProposal = BossPhaseAuthority.prototype.preparePhaseExit.call(
      boss,
      observe.id,
    ...
    if (
      ...
      || laserProposalView.eventBus !== eventBus
    ) {
    ...
    this.currentTick120 = startTick120;
  ...
  /** Prepare one exact tick; the caller closes it with the single flush. */
  ...
  flushTick(tick120Value: number): readonly CanonicalGameplayEvent[] {
    ...
    const tick120 = requireSafeTick(tick120Value, "Misreader flush tick120");
    if (tick120 !== this.currentTick120) {
      throw new Error(
        `Misreader laser can flush only its current tick: ${this.currentTick120} !== ${tick120}`,
      );
    }
    if (this.pendingFlushTick120 !== tick120) {
      throw new Error(`Misreader tick ${tick120} has no prepared flush`);
    }
    try {
  ...
  private assertOperational(): void {
    ...
    throw new Error(
      `Misreader laser fragment is faulted: ${this.fault.message}`,
      {cause: this.fault},
    );
  }
  ...
  private advanceTickUnlocked(
    ...
    this.assertOperational();
    if (this.pendingFlushTick120 !== null) {
      throw new Error(`Misreader tick ${this.pendingFlushTick120} must flush before advancing`);
    }
    const validated = validateCombatStep(input, {
    ...
    const overrideBefore = this.override.snapshot();
    ...
    try {
      ...
      this.override.advanceTo(validated.tick120);
...
export class CanonicalCombatKernel {
  ...
  constructor(
    ...
    if (!isExactCanonicalEventBus(eventBus)) {
      throw new Error("canonical combat requires an exact CanonicalEventBus instance");
    }
    ...
    const initialPlayerPosition = suppliedInitialPlayerPosition
    ...
    const occurrenceId = captured.occurrenceId === undefined
      ? patternId
    ...
    const projectilePoolClasses = captureProjectilePoolClasses(
      captured.projectilePoolClasses,
      "canonical combat projectilePoolClasses",
    );
    if (
  ...
  /**
   * Mutate one exact master tick without closing the event bus. A coordinator
   * may enqueue other same-tick authority facts before the single flush.
   */
  ...
  flushTick(tick120Value: number): readonly CanonicalGameplayEvent[] {
    this.assertOperational();
    const tick120 = requireSafeTick(tick120Value, "combat flush tick120");
    if (tick120 !== this.currentTick120) {
      throw new Error(
        `canonical combat can flush only its current tick: ${this.currentTick120} !== ${tick120}`,
      );
    }
    if (this.pendingFlushTick120 !== tick120) {
  ...
  private assertOperational(): void {
    if (this.fault === null) return;
    throw new Error(
      `canonical combat kernel is faulted: ${this.fault.message}`,
      {cause: this.fault},
    );
  }
  ...
  private advanceTickUnlocked(input: CanonicalCombatStepInput): CanonicalCombatSnapshot {
    this.assertOperational();
    if (this.pendingFlushTick120 !== null) {
      throw new Error(`combat tick ${this.pendingFlushTick120} must flush before advancing`);
    }
    ...
    try {
      ...
      this.pendingFlushTick120 = tick120;
      return this.snapshot();
    } catch (error) {
      this.fault = error instanceof Error ? error : new Error(String(error));
      throw error;
  ...
  private validateStepInput(input: CanonicalCombatStepInput): ValidatedCombatStepInput {
    return validateCombatStep(input, {
      currentTick120: this.currentTick120,
      currentPlayerPosition: this.currentPlayerPosition,
      player: this.player,
    });
