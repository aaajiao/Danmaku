...
describe("same-tick composition with a player release", () => {
  it("orders a completion before a same-tick player collision-on on the shared bus", () => {
    ...
    expect(authority.advance(78)).toMatchObject({state: "idle", targetRoom: null, eventCount: 4});
    bus.enqueue({
      id: "player.collision.on",
      tick120: 78,
      entityStableId: "player:0",
      localSequence: 0,
      occurrenceKey: "room-transition-test:player-release",
      payload: {owner: "room-transition", reason: "atomic-world-swap"},
    });
