...
export class RoomTransitionAuthority {
  ...
  request(targetRoomValue: unknown, tick120Value: unknown): RoomTransitionAuthoritySnapshot {
    ...
    // The event batch validates before any machine field changes.
  ...
  advance(tick120Value: unknown): RoomTransitionAuthoritySnapshot {
    ...
    // A large advance commits all crossed events as one validated batch. Each
    // draft retains its authored due tick rather than borrowing the caller tick.
