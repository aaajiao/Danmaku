...
- authority与focused producer：`953159e`；新增exact material hold port，并把`successor-next-material`纳入
  Run不可逆flush前的owner、四lineage、pool、room、player与Override同步验证。
- focused Vitest：同一真实producer通过，1 passed / 8 skipped；global `8520..8682`产生63组相邻cleanup pair，
  最后一个remove位于`8682`，`8683` empty hold不产生material event。
- strict typecheck与`git diff --check`通过；按冻结计划未运行build、smoke、E2E或全unit。
- 只读边界复审确认flush前验证闭合，无剩余阻断问题。
...
ACCEPTED。已经转交的材料在章节关闭后继续其自身时间，同时让现场缺席和allocation仍被分别记录；第三
