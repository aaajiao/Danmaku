...
| `src/authority/run-behavior-capture.ts` | Danmaku / aaajiao + Codex | Bun 1.3.14 / TypeScript | H+1702 exact closure baseline | repository license | `9c150389b1e6dd8a36729aa5b2214c3c98abfb3f27d82900774100ca3fb27620` |
...
- verification budget：pure derivation test目标低于1秒；一条真实H+1702 producer/wrapper integration约3秒；
  focused projection + direct Run consumer总计保持30秒内。只有closure/session/ledger producer发生变化才升级
  EXT-009直接关联suite。
- 一条真实Run到H+1702，固定closure bytes/hash，证明projector只接受module-issued receipt；H+1703重复
  projection不变，event bytes、selection/RNG/target/transition均不变。
- test-only immutable exact-schema fixture只测试unbranded pure core：14项exact order、两个公式、12个missing、
  deep freeze、byte determinism、wrong boundary/content、zero denominator与metric-universe drift；fixture bytes/hash
  必须与真实producer case相等，production不得import。
- wrapper focused cases覆盖fake receipt与plain clone拒绝；pure payload不能被正式consumer接受。
- strict typecheck、`bun run content:check`、`bun run build`与`git diff --check`；没有UI、selection、transition或
  handoff可见变化，因此不跑browser/smoke/full E2E/`test:all`。
...
PROPOSED。H+1702只投影`avgFlower`与committed `clamped` state `gazeRatio`；其余12项保留typed missing，所有
