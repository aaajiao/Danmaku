...
- 分支 / PR：`agent/canonical-run-integration` / pending
- 实现提交：pending
...
| application implementation/tests | Danmaku / aaajiao + Codex | Bun 1.3.14 / TypeScript | this decision | repository license | pending until ACCEPTED |

V4 source tree保持只读。实现提交与 application artifact hashes 只能在验证完成、状态改为 ACCEPTED 时写入。

## 验证计划

- exact V4 manifest/projection/source validation，canonical ID universe 不变；
- Input focused tests：KeyG、gamepad button 3、双 pointer、单 pointer release、lost capture、blur 与 gamepad
  disconnect 不得 latch；
- Run-session focused tests：release+29 不完成、+30 commit、re-clamp invalidation/re-anchor、四 barrier 与
  frozen `ready_for_room_sampling` snapshot；
- 相同 seed/input 的 canonical serialization 相同，Full == Reduced Motion == Flash-Off gameplay trace；
- production-preview E2E 实际 hold/release 到 typed `ROOM_SAMPLING` ready；
- `content:check`、focused Vitest、strict typecheck、build 与 `git diff --check`；
- pool/RNG/asset budget 为零。未记录物理设备矩阵前不作实机兼容声明。
...
PROPOSED。接受条件是：上述三类设备 route、exact tick recovery、re-clamp policy、typed handoff、profile
parity 与 production-preview E2E 全部通过，并补齐实现提交和 artifact hashes。尚未解决的边界是
