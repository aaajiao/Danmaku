...
transaction中推进两个已drain lineage、Misregistration residue、room/player并sole-flush；只允许原entity的
cleanup event。composer rest在`8327`开始，slice必须在`8519`关闭，即使真实producer仍有63个residue；不得等待
`8682` drain或强制清屏。close后零tick/event/RNG退休旧Context Switch material lease，并把Misregistration原
pool、identity、deadline和80-slot lease交给新的opaque material owner。契约见
[EXT-2026-023](adr/EXT-2026-023-second-in-between-material-tail-transfer.md)；实现状态以Roadmap为准，Session、第三
occurrence、room completion与handoff继续withheld。
