...
| Canonical Run | WIP | 首房关闭 → transition/material → successor telegraph/entry/READ → pattern end释放occurrence → slice-close material transfer → 同房encounter ordinal 1 dormant admission已闭合；下一tick执行、Session消费与room completion/handoff仍未完成 |
...
| P0-05 | Live room composer | WIP | EXT-012 target、EXT-013 transition及EXT-015—017 successor plan/terminal/session已闭合首个ordinal 1 live slice；EXT-018—020已闭合下一合法pattern能力、post-slice source及draw 2 → Misregistration dormant admission。下一步实现双pool next-tick coordinator；Session与room completion仍withheld |
...
1. 从EXT-020 dormant owner的下一accepted tick开始推进Misregistration telegraph/entry/READ：同tick先推进retained
   collisionless material与新combat，再由Run sole-flush；READ边界才claim occurrence。先做chapter coordinator，
   不等待residue drain、不重抽pattern，也不顺带授权第三occurrence或room completion；完成后再接Session。
