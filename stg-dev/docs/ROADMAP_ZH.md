...
| Canonical Run | WIP | 首房关闭 → transition/material → successor telegraph/entry/READ → pattern end释放occurrence → slice-close material transfer已闭合；room completion/handoff与next-occurrence Session消费仍未授权 |
...
| Room composition / execution | WIP（首个 live 切片） | ordinal 1首occurrence已接入Run并推进到exact slice close；EXT-019建立了保留原pool/identity和allocated-slot lease的sealed material source。下一plan/admission、Session消费、room count/order与完整multi-pool仍未完成 |
...
| P0-05 | Live room composer | WIP | EXT-012 target、EXT-013 transition及EXT-015—017 successor plan/terminal/session已闭合首个ordinal 1 live slice；EXT-018移除合法下一pattern的executor阻塞，EXT-019闭合post-slice sealed material transfer。下一步消费该source提交同房next-occurrence plan/admission；Session、多pool与room completion仍withheld |
...
| P0-07 | Canonical Run / narrative | WIP | awakening、First Eye、固定首房、captures、partial metrics、target、Room Threshold及successor首slice已进入同一session/只读presentation；post-slice transfer目前只是下一准入可消费的sealed边界，尚未接入Session。下一步关闭目标房的后续occurrence/room handoff，完整Run终点仍未授权 |
...
1. 消费EXT-019 sealed material source，提交同房next-occurrence plan/admission；按retained allocated slots
   计算预算，材料lease只在显式release后归还。先闭合实际选择所需的pool join，不把本次边界冒充Session或
   完整multi-pool完成；不得等待residue drain、清空材料、重抽pattern或扩大已commit预算，room completion
   继续独立withheld。
