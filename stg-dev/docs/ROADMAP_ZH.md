...
| Canonical Run | WIP | 独立authority已完成首房关闭 → transition/material → telegraph/entry → 下一房READ → pattern end释放occurrence → collisionless residue跨精确slice边界保留；尚未接回session，slice complete仍不授予room completion/handoff |
...
| Room composition / execution | WIP（首个 live 切片） | ordinal 1首occurrence从plan/admission到READ、material-settle/rest与exact slice close已闭合；EXT-016在digital body排空后释放gameplay claim，并保留未到期residue。后续session接线、material handoff、room count/order与完整multi-pool仍未完成 |
...
| P0-05 | Live room composer | WIP | EXT-012 target、EXT-013 transition、EXT-015 admission/READ及EXT-016 terminal material已闭合首个ordinal 1 occurrence slice；下一步接回session，并为retained residue建立下一消费边界，room completion仍withheld |
...
| P0-07 | Canonical Run / narrative | WIP | awakening、First Eye、固定首房、captures、partial metrics、target、Room Threshold/material handoff及successor完整首slice已闭合；下一步把telegraph/entry/READ/tail接回session与只读presentation，后续room completion仍未授权 |
...
1. 把完成的telegraph/entry/READ/material tail接回session和renderer；建立只引用V4唯一源文件的共享/章节素材registry，
   不复制二进制素材，不把preview/QA图当runtime资产，并让构建产物继续由部署阶段生成。
2. 为slice complete仍在场的collisionless residue建立下一消费边界，再明确room completion/next occurrence；
