...
| Renderer / input / PWA | WIP（首个因果素材族） | shared/chapter runtime registry已闭合7/7图集与448 frame atlas依赖，当前接4个房间背景、4个room bed和6个既有feedback音效；First Eye已按正式事件/状态投影reveal、clamp、Reduced Motion fallback、release及一次clamp音频/触觉。其余事件族、动态clip、reaction、升级迁移和实机矩阵未完成 |
...
| P0-09 | Presentation / accessibility | WIP（首个因果素材族） | `stg-dev/src/assets`已让共享层唯一绑定V4物理URL、章节层只选择ID，并闭合7张正式图集；First Eye steady frame、Reduced Motion fallback、clamp音频/触觉已从真实Run提交事实投影。下一缺口是projectile/threshold/Flower等事件族；动态clip、crossfade与reaction须先闭合其组合规则。完整Run的full/reduced-motion/flash-off gameplay trace仍须证明相同 |
...
1. 沿进入真实Run的章节事件继续补projectile、threshold与Flower等被动素材投影；First Eye当前只闭合V4
   已定义的steady frame、音频与触觉，不冒充动态clip。动态playhead、crossfade与reaction组合规则先走focused
   决定；章节只选择共享V4 ID，不复制素材，不把preview/QA图当runtime资产，`dist`继续由部署阶段生成。
